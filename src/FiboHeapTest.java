...
public class FiboHeapTest {
	...
	static class DecreasedKey {
		...
		public String toString() {
			return "DecreasedKey{" + "key=" + key + ", delta=" + delta + '}';
		}
	}
	...
	static class DecreasedKeyException extends Exception {
		...
		public DecreasedKeyException(List<DecreasedKey> keys, String msg,
				Throwable ex) {
			super(msg, ex);
			this.keys = keys;
		}
	...
	public static CreateHeapResult createHeap(int from, int to,
			boolean testAfterInsert) {
		List<Integer> l = IntStream.rangeClosed(from, to).boxed()
	...
	public static int expectedNumOfTrees(int size) {
		return (int) Integer.toBinaryString(size).chars()
				.filter(ch -> ch == '1').count();
	}

	public static Map<Integer, FibonacciHeap.HeapNode> testInserts(
			FibonacciHeap heap, List<Integer> keys) {
		int size = heap.size();
		...
		for (Integer n : keys) {
			...
			testFibHeap(heap);
			size++;
			testHeapFields(heap, new ExpectedFields(size, null, (int) minKey));
		}
		return nodes;
	}

	public static void testDeletes(FibonacciHeap heap,
			List<Integer> orderedKeys) throws DeleteException {
		int size = heap.size();
		Integer expectedSize;
		for (Integer minVal : orderedKeys) {
			if (heap.findMin().getKey() != minVal) {
				throw new RuntimeException(String.format(
						"wrong minimum value found. should have been %d but was %d",
						minVal, heap.findMin().getKey()));
			}
			expectedSize = size == heap.size() ? null
					: expectedNumOfTrees(size);
			testHeapFields(heap,
					new ExpectedFields(size, expectedSize, minVal));
			try {
				heap.deleteMin();
			} catch (Exception ex) {
				throw new DeleteException(heap.findMin().getKey(),
						"error deleting key " + heap.findMin());
			}
			size--;
		}
	}
	...
	public static int getNumMarked(FibonacciHeap heap) {
		return (heap.potential() - heap.numTrees) / 2;
	}

	public static void testDecreaseKey(FibonacciHeap heap,
			Map<Integer, FibonacciHeap.HeapNode> nodes, int amount)
			throws DecreasedKeyException {
		int randIndex, delta;
		FibonacciHeap.HeapNode[] nodesArr = new FibonacciHeap.HeapNode[nodes
				.size()];
		List<DecreasedKey> decreasedKeys = new ArrayList<>();
		int i = 0;
		for (Map.Entry<Integer, FibonacciHeap.HeapNode> entry : nodes
				.entrySet()) {
		...
		try {
			...
			throw new DecreasedKeyException(decreasedKeys,
					"Error during decreasedKey", ex);
		}
	}
	...
	public static void testMeld(FibonacciHeap heap, int upperBound) {
		...
		int size = heap.size(), numOfTrees = heap.numTrees,
				numMarked = getNumMarked(heap);
		heap.meld(heap2);
		if (heap.size() != size + heap2.size()) {
			throw new RuntimeException(
					"size after meld is not sum of both heaps");
		}
		if (heap.numTrees != numOfTrees + heap2.numTrees) {
			throw new RuntimeException(
					"numOfTrees after meld is not sum of both heaps");
		}
		if (getNumMarked(heap) != numMarked + getNumMarked(heap2)) {
			throw new RuntimeException(
					"numMarked after meld is not sum of both heaps");
		}
		testFibHeap(heap);
	}
	...
	public static void testKMin(FibonacciHeap heap) {
		if (heap.size() == 0) {
			...
			if (kMinVals.length != 0) {
				throw new RuntimeException(
						"kMin did not return empty array for empty heap");
			}
		} else {
			...
			FibonacciHeap heaps[] = new FibonacciHeap[heap.numTrees];
			int heapIndex = 0;
			do {
				next = cur.getNext();
				cur.setPrev(cur);
				...
				heaps[heapIndex].min = cur;
				heaps[heapIndex].first = cur;
				heaps[heapIndex].last = cur;
				heaps[heapIndex].size = FiboHeapTest.countTreeItems(cur);
				heaps[heapIndex].numTrees = 1;
	...
	public static void testRootPointers(FibonacciHeap heap) {
		...
		do {
			...
			if (cur.getNext().getPrev() != cur)
				throw new RuntimeException(
						"getNext.getPrev did not return current root: "
								+ cur.getKey());
			if (cur.getPrev().getNext() != cur)
				throw new RuntimeException(
						"getPrev.getNext did not return current root: "
								+ cur.getKey());
			if (cur.getParent() != null) {
				throw new RuntimeException(
						String.format("tree root %d has parent", cur.getKey()));
			}
			cur = cur.getNext();
	...
	public static void testRanks(FibonacciHeap.HeapNode treeRoot) {
		...
		if (rank == 0) {
			if (child != null) {
				throw new RuntimeException("rank is zero but node "
						+ treeRoot.getKey() + " has child " + child.getKey());
			}
		} else {
			do {
				childrenAmount++;
				if (curNode.getChild() != null)
					testRanks(curNode.getChild());
	...
	public static void testFibMin(FibonacciHeap heap) {
		...
		do {
			if (cur.getKey() < heap.findMin().getKey()) {
				throw new RuntimeException(
						String.format("minimum must be %s but is %s",
								heap.findMin().getKey(), cur.getKey()));
			}
			cur = cur.getNext();
		} while (cur != heap.first);
	}
	...
	public static int calculateNumOfTrees(FibonacciHeap heap) {
		...
		FibonacciHeap.HeapNode cur = heap.first.getNext();
		while (cur != heap.first) {
			i++;
			cur = cur.getNext();
		}
	...
	public static void testNumOfTrees(FibonacciHeap heap) {
		...
		if (actualNumOfTrees != heap.numTrees) {
			throw new RuntimeException(
					String.format("should have %s trees but counted %s",
							heap.numTrees, actualNumOfTrees));
		}
	}
	...
	public static void testFibHeap(FibonacciHeap heap) {
		...
		testFibMin(heap); // Minimum must be one of the roots otherwise
							// testHeapMinProp would have failed
	}
	...
	public static void testSiblings(FibonacciHeap.HeapNode root) {
		if (root.getChild() != null) {
			...
			do {
				if (cur.getPrev().getNext() != cur)
					throw new RuntimeException(
							"getPrev.getNext did not return current child: "
									+ cur.getKey());
				if (cur.getNext().getPrev() != cur)
					throw new RuntimeException(
							"getNext.getPrev did not return current child: "
									+ cur.getKey());
				cur = cur.getNext();
			} while (cur != child);
		}
	...
	public static void testChildren(FibonacciHeap.HeapNode node) {
		if (node.getChild() != null) {
			if (node.getChild().getParent() != node)
				throw new RuntimeException(
						"getChild.getParent did not return cur node: "
								+ node.getKey());
			...
			do {
				if (cur.getParent() != node)
					throw new RuntimeException(
							"getParent of a sibling did not return current node: "
									+ node.getKey());
				cur = cur.getNext();
			} while (cur != child);
		}
		...
		if (node.getChild() != null) {
			...
			do {
				if (curChild.getKey() < node.getKey())
					throw new RuntimeException(
							"child found with smaller key than parent: "
									+ node.getKey());
	...
	private static int countTreeItemsRec(FibonacciHeap.HeapNode child) {
		...
		do {
			...
			if (cur.getChild() != null)
				cnt += countTreeItemsRec(cur.getChild());
			cur = cur.getNext();
		} while (cur != child);
		return cnt;
	...
	public static void testHeapFields(FibonacciHeap heap,
			ExpectedFields expected) {
		if (expected.numOfTrees != null
				&& expected.numOfTrees != heap.numTrees) {
			throw new RuntimeException(String.format(
					"wrong amount of trees. found %d but expected %d",
					heap.numTrees, expected.numOfTrees));
		}
		if (expected.size != heap.size()) {
			throw new RuntimeException(String.format(
					"wrong size of tree. found %d but expected %d", heap.size(),
					expected.size));
		}
		if (expected.minKeyVal == null) {
			if (heap.findMin() != null) {
				throw new RuntimeException(String.format(
						"wrong minimum found. found %d but expected null",
						heap.findMin().getKey()));
			}
		} else {
			if (expected.minKeyVal != heap.findMin().getKey()) {
				throw new RuntimeException(String.format(
						"wrong min key in tree. found %d but expected %d",
	...
	public static void testRandomHeap(int size, int jumps) throws Exception {
		List<Integer> keys = IntStream.rangeClosed(0, size).boxed()
				.map(x -> x * jumps).collect(Collectors.toList());
		...
		try {
			...
			heap = new FibonacciHeap();
			Map<Integer, FibonacciHeap.HeapNode> nodes = testInserts(heap,
					keys);
			...
			System.out.println(
					"failed deleting minimum when min = " + ex.deleteKey);
			throw ex;
